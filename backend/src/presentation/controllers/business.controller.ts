...
businessRouter.post('/logo', requireBusinessRole(['OWNER']), upload.single('image'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
    ...
    if (!allowedTypes.includes(req.file.mimetype)) {
      res.status(400).json({ error: 'Invalid file type', message: 'Only JPEG, PNG, GIF, and WebP images are allowed' });
      return;
    }
...
businessRouter.put('/onboarding', requireBusinessRole(['OWNER']), async (req: Request, res: Response) => {
  try {
